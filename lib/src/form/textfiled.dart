...
/// 输入框事件回调
typedef JSCompletedCallBack = void Function(String content);
typedef JSClearCallBack = void Function();
...
enum JSClearDisplayMode {
  whileEditing, // 编辑的时候出现
  always, // 一致出现
}

// ignore: must_be_immutable
class JSTextField extends StatefulWidget {
  /// 键盘样式
  final JSKeyBoardType? keyboardType;

  final JSClearDisplayMode? clearDisplayMode;
  ...
  /// 输入回调
  final JSCompletedCallBack fieldCallBack;

  /// 取消回调
  final JSClearCallBack? clearCallBack;
  ...
  JSTextField(
    this.hintText, {
    Key? key,
    JSKeyBoardType keyboardType: JSKeyBoardType.text,
    ...
    this.focusCallBack,
        ...
        keyboardType = maxLines == 1 ? keyboardType : JSKeyBoardType.multiline,
        super(key: key);
...
class _SMTextFieldFieldState extends State<JSTextField> {
  ...
  TextInputType _getTextInputType() {
    switch (widget.keyboardType) {
      case JSKeyBoardType.text:
        return TextInputType.text;
      case JSKeyBoardType.multiline:
        return TextInputType.multiline;
      case JSKeyBoardType.number:
        _isNumber = true;
        return TextInputType.number;
      case JSKeyBoardType.specialNumber:
        _specialNumber = true;
        return TextInputType.number;
      case JSKeyBoardType.phone:
        _isNumber = true;
        return TextInputType.phone;
      case JSKeyBoardType.datetime:
        return TextInputType.datetime;
      case JSKeyBoardType.emailAddress:
        return TextInputType.emailAddress;
      case JSKeyBoardType.url:
        return TextInputType.url;
      case JSKeyBoardType.password:
        return TextInputType.text;
      case JSKeyBoardType.verifyCode:
        _isVerifyCode = true;
        return TextInputType.number;
      default:
        return TextInputType.text;
  ...
  List<TextInputFormatter> _onVerifyTextFieldFormatter() {
    ...
    if (widget.keyboardType == JSKeyBoardType.password) {
      return <TextInputFormatter>[
  ...
  Widget build(BuildContext context) {
    ...
    JSClearDisplayMode clearDisplayMode = widget.clearDisplayMode ?? JSClearDisplayMode.whileEditing;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: TextStyle(
            color: widget.hintColor ?? themeColors.fontTips(),
        ...
        style: TextStyle(
          color: widget.textColor ?? themeColors.fontMain(),
   ...
   */
  Widget _getClearButton({required JSClearDisplayMode clearDisplayMode, Widget? clearWidget, required bool showClear}) {
    Widget widget;
    switch (clearDisplayMode) {
      case JSClearDisplayMode.whileEditing:
        if (showClear) {
          widget = _inputText.length > 0
              ? (clearWidget ??
                  Container(
                    ...
                    decoration: BoxDecoration(color: themeColors.line(), borderRadius: BorderRadius.circular(8)),
                  ))
              : Container();
        } else {
          widget = Container();
        ...
        return widget;
      case JSClearDisplayMode.always:
        if (showClear) {
          widget = (clearWidget ??
              Container(
                width: 16,
