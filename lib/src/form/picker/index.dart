...
class JSPicker {
  ...
  void show(
    ...
    JSModal.show(context: context, child: picker);
  ...
  Future<List<int>?> showDialog(BuildContext context, {bool barrierDismissible = true, Color? backgroundColor, PickerWidgetBuilder? builder, Key? key}) {
    return Dialog.showDialog<List<int>>(
        ...
        builder: (BuildContext context) {
          ...
          if (cancel == null) {
            String? _cancelText = cancelText ?? '取消';
            if (_cancelText != "") {
          ...
          if (confirm == null) {
            String? _confirmText = confirmText ?? '确认';
            if (_confirmText != "") {
              actions.add(TextButton(
                  style: _getButtonStyle(ButtonTheme.of(context)),
                  onPressed: () async {
          ...
          return AlertDialog(
            ...
            backgroundColor: backgroundColor,
            actions: actions,
            content: builder == null ? makePicker() : builder(context, makePicker()),
          );
        });
  }
  ...
  */
  ...
  static ButtonStyle _getButtonStyle(ButtonThemeData? theme) => ButtonStyle(minimumSize: MaterialStateProperty.all(Size(theme?.minWidth ?? 0.0, 42)), padding: MaterialStateProperty.all(theme?.padding));
}
...
class PickerWidgetState<T> extends State<_PickerWidget> {
  ...
  */
  Widget _buildHeaderButton(BuildContext context, {required String text, required VoidCallback onPressed, TextStyle? style}) {
    return TextButton(
        style: JSPicker._getButtonStyle(ButtonTheme.of(context)),
        onPressed: onPressed,
        child:
            Text(text, overflow: TextOverflow.ellipsis, textScaleFactor: MediaQuery.of(context).textScaleFactor, style: style ?? theme!.textTheme.button!.copyWith(color: theme!.colorScheme.secondary, fontSize: JSPicker.DefaultTextSize)));
  }
  ...
  List<Widget> _buildHeaderViews(BuildContext context) {
    ...
    if (picker.cancel != null) {
      items.add(DefaultTextStyle(style: picker.cancelTextStyle ?? theme!.textTheme.button!.copyWith(color: theme!.colorScheme.secondary, fontSize: JSPicker.DefaultTextSize), child: picker.cancel!));
    } else {
      ...
      child: picker.title == null
          ...
          : DefaultTextStyle(
              style: theme!.textTheme.headline6!.copyWith(
                fontSize: JSPicker.DefaultTextSize,
              ),
              child: picker.title!),
    )));

    if (picker.confirm != null) {
      items.add(DefaultTextStyle(style: picker.confirmTextStyle ?? theme!.textTheme.button!.copyWith(color: theme!.colorScheme.secondary, fontSize: JSPicker.DefaultTextSize), child: picker.confirm!));
    } else {
      String? _confirmText = picker.confirmText ?? '确认';
      if (_confirmText != "") {
  ...
  void updateScrollController(int i) {
    ...
    for (int j = 0; j < picker.selecteds.length; j++) {
      if (j != i) {
        if (scrollController[j].hasClients && scrollController[j].position.hasContentDimensions) scrollController[j].position.notifyListeners();
      }
    }
    _changing = false;
  }
